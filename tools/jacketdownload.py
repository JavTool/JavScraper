...
读取 nfo 中的 originaltitle、sorttitle 和 studio 字段，从 originaltitle、sorttitle 提取番号，从 studio 提取工作室 ，若工作室为 "カリビアンコム" 则继续执行以下操作
根据番号从 caribbeancom 中 下载封面图片
...
class JacketDownloader:
        ...
        try:
            ...
            for field in ['originaltitle', 'sorttitle']:
                ...
                if elem is not None and elem.text:
                    ...
                    match = re.search(r'(\d{6}-\d{3})', text)
                    if match:
            ...
            match = re.search(r'(\d{6}-\d{3})', filename)
            if match:
                return match.group(1)
    ...
    def download_caribbean_jacket(self, number: str, save_dir: str) -> bool:
        """从 Caribbean.com 下载封面图片"""
        try:
            # Caribbean.com 封面图片 URL 格式
            # 例如：123456-789 -> https://www.caribbeancom.com/moviepages/011324-001/images/jacket.jpg
            jacket_url = f"https://www.caribbeancom.com/moviepages/{number}/images/jacket.jpg"
    ...
    def download_caribbean_poster(self, number: str, save_dir: str) -> bool:
        """从 Caribbean.com 下载封面图片"""
        try:
            # Caribbean.com 封面图片 URL 格式
            # 例如：123456-789 -> https://www.caribbeancom.com/moviepages/011324-001/images/l_l.jpg
            poster_url = f"https://www.caribbeancom.com/moviepages/{number}/images/l_l.jpg"
            
            self.log_message(f"尝试下载封面: {poster_url}")
            ...
            # 保存为 poster.jpg
            poster_path = os.path.join(save_dir, "landscape.jpg")
            with open(poster_path, 'wb') as f:
                f.write(response.content)
            
            self.log_message(f"成功下载封面: {poster_path}")
            
            # 复制为 thumb.jpg 和 fanart.jpg
            thumb_path = os.path.join(save_dir, "thumb.jpg")
            fanart_path = os.path.join(save_dir, "fanart.jpg")
            
            try:
                shutil.copy2(poster_path, thumb_path)
                self.log_message(f"复制封面为: {thumb_path}")
            except Exception as e:
                self.log_message(f"复制 thumb.jpg 失败: {e}")
            
            try:
                shutil.copy2(poster_path, fanart_path)
                self.log_message(f"复制封面为: {fanart_path}")
            except Exception as e:
                self.log_message(f"复制 fanart.jpg 失败: {e}")
    ...
    def process_nfo_file(self, nfo_path: str) -> bool:
        ...
        # 2. 检查是否为 Caribbean.com
        if studio != "カリビアンコム":
            self.log_message(f"工作室不是 カリビアンコム ({studio})，跳过: {nfo_path}")
            return False
        ...
        landscape_path = os.path.join(nfo_dir, "landscape.jpg")
        
        if os.path.exists(landscape_path):
            self.log_message(f"landscape.jpg 已存在，跳过下载: {landscape_path}")
            return True
        
        # 5. 下载封面图片
        success = self.download_caribbean_poster(number, nfo_dir)
    ...
    def process_directory(self, directory: str) -> dict:
        ...
        self.log_message(f"Caribbean.com NFO 文件数: {stats['caribbean_nfo']}")
