...
namespace JavScraper.App
{
    public class VideoTools
    {
        ...
        public static string GetPicFromVideo(string videoName, string CutTimeFrame, string PicName)
        {
            //string PicName = Server.MapPath("~/video/" + Guid.NewGuid().ToString().Replace("-", "") + ".jpg");
             
            ProcessStartInfo startInfo = new ProcessStartInfo(ffmpegPath);
            startInfo.WindowStyle = ProcessWindowStyle.Hidden;
            startInfo.Arguments = " -ss " + CutTimeFrame + " -t 0.001 -i \"" + videoName + "\" -y -f image2 \"" + PicName+ "\"";  //設定程式執行參數
            try
            {
                //videoTimeLength = 0;
