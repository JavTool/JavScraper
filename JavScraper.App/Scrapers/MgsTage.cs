...
namespace JavScraper.App.Scrapers
{
    ...
    public class MgsTage : AbstractScraper
    {
        ...
        public MgsTage(ILoggerFactory loggerFactory)
            : base("https://www.mgstage.com/", loggerFactory.CreateLogger<MgsTage>())
        {
        }
        ...
        /// <returns></returns>
        public override async Task<List<JavVideo>> ParseList(List<JavVideo> ls, HtmlDocument doc)
        {
            if (doc == null)
                return ls;
            ...
            foreach (var node in nodes)
            {
                ...
                var m = new JavVideo()
                {
                    ...
                    Number = url.Split("/".ToArray(), StringSplitOptions.RemoveEmptyEntries).Last(),
        ...
        public override async Task<JavVideo> ParsePage(string url)
        {
            ...
            if (string.IsNullOrWhiteSpace(m.Plot))
                m.Plot = await GetDmmPlot(m.Number);
