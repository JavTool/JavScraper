...
namespace JavScraper.Tools.Services
{
    ...
    public class FixNfoService
    {
        ...
        private readonly ILogger<FixNfoService> _logger;
        ...
        private readonly TagReplaceConfig _tagReplaceConfig;
        ...
        public FixNfoService(ILoggerFactory loggerFactory)
        {
            _loggerFactory = loggerFactory;
            _logger = loggerFactory.CreateLogger<FixNfoService>();
            ...
            _tagReplaceConfig = TagReplaceConfig.LoadFromFile();
        }

        /// <summary>本地 JavVideo 缓存目录，程序运行目录下的 data 子目录。</summary>
        private static readonly string DataCacheDir =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data");

        /// <summary>
        /// 尝试从本地缓存读取 <see cref="JavVideo"/>。
        /// </summary>
        private static async Task<Entities.JavVideo> TryGetCachedJavVideoAsync(string javId)
        {
            try
            {
                var file = Path.Combine(DataCacheDir, $"{javId}.json");
                if (!File.Exists(file))
                    return null;
                var json = await File.ReadAllTextAsync(file);
                return JsonSerializer.Deserialize<Entities.JavVideo>(json);
            }
            catch
            {
                return null;
        ...
        /// <summary>
        /// 将 <see cref="JavVideo"/> 序列化并保存到本地缓存。
        /// </summary>
        private static async Task SaveJavVideoCacheAsync(string javId, Entities.JavVideo javVideo)
        {
            try
            {
                Directory.CreateDirectory(DataCacheDir);
                var file = Path.Combine(DataCacheDir, $"{javId.ToUpper()}.json");
                var options = new JsonSerializerOptions { WriteIndented = true };
                var json = JsonSerializer.Serialize(javVideo, options);
        ...
        // 目录名格式正则：匹配 "[2016-05-28] - [番号] - [标题]" 或带前缀 "[中字] - ..." 等格式
        private static readonly Regex DirNameFormatRegex = new Regex(
            @"^(?:\[[^\]]+\]\s*-\s*)?\[(\d{4}-\d{2}-\d{2})\]\s*-\s*\[[^\]]+\]\s*-\s*\[[^\]]+\]$",
            RegexOptions.Compiled);

        // sorttitle 格式正则：匹配 "[番号] - [标题]" 格式
        private static readonly Regex SortTitleFormatRegex = new Regex(
            @"^\[[^\]]+\]$",
            RegexOptions.Compiled);

        /// <summary>
        /// 修复模式：遍历指定路径下符合目录名格式的 NFO 文件，
        /// 若同目录存在 movie.nfo 则从中复制字段，否则调用 ProcessNfoFile 进行爬取修复。
        /// </summary>
        ...
        public async Task FixNfoFilesAsync(string path)
        {
            ...
            // 筛选出符合目录名格式的主 nfo 文件（非 movie.nfo、非备份）
            var targetNfoFiles = javFiles.Keys
                .Where(filePath =>
                {
                    var ext = Path.GetExtension(filePath);
                    if (!ext.Equals(".nfo", StringComparison.OrdinalIgnoreCase))
                        return false;
                    if (filePath.Contains(".bak.nfo", StringComparison.OrdinalIgnoreCase))
                        return false;
                    var fileName = Path.GetFileNameWithoutExtension(filePath);
                    if (string.Equals(fileName, "movie", StringComparison.OrdinalIgnoreCase))
                        return false;
                    // 文件名需符合目录格式正则
                    if (!DirNameFormatRegex.IsMatch(fileName))
                        return false;
                    // 检查 nfo 内的 actor 字段是否包含英文中括号 "[" 或 "]"
                    //try
                    //{
                    //    //var doc = new NfoDocument(filePath);
                    //    //var releaseDate = doc.GetReleaseDate();
                    //    //// 如果发行日期是 2023-10-15 则直接返回 true
                    //    //if (!string.IsNullOrEmpty(releaseDate) && releaseDate.Trim() == "2023-10-15")
                    //    //    return true;
                    //    //else
                    //    //    return false;
                    //    // var actors = doc.GetActors();
                    //    // return actors != null && actors.Any(a =>
            ...
            Console.WriteLine($"共找到 {targetNfoFiles.Count} 个需要处理的 NFO 文件。");

            int index = 0;
            foreach (var nfoFilePath in targetNfoFiles)
            {
                index++;
                Console.WriteLine($"[{index}/{targetNfoFiles.Count}] 处理: {nfoFilePath}");
                try
                {
                    var dir = Path.GetDirectoryName(nfoFilePath);
                    var movieNfoPath = Path.Combine(dir, "movie.nfo");

                    if (File.Exists(movieNfoPath))
                    {
                        // 从 movie.nfo 复制字段到目录名同名 nfo
                        await CopyFromMovieNfo(movieNfoPath, nfoFilePath);
                    }
                    else
                    {
                        // 调用爬取修复
                        await ProcessNfoFile(nfoFilePath);
                    }
                }
                catch (Exception ex)
                {
                    LogError($"处理文件 {nfoFilePath} 时发生错误", ex);
                }
            }
        ...
        /// <summary>
        /// 从 movie.nfo 复制 title、originaltitle、sorttitle、actor、genre、tag 字段到目标 nfo，
        /// 并按照条件规则决定是否更新每个字段。
        /// </summary>
        private async Task CopyFromMovieNfo(string movieNfoPath, string targetNfoPath)
        {
            var movieDoc = new NfoDocument(movieNfoPath);
            var targetDoc = new NfoDocument(targetNfoPath);

            var movieTitle = movieDoc.GetTitle();
            var movieOriginalTitle = movieDoc.GetOriginalTitle();
            var movieSortTitle = movieDoc.GetSortTitle();
            var movieActors = movieDoc.GetActors(); // List<(string Name, string Type)>
            var movieGenres = movieDoc.GetGenres();
            var movieTags = movieDoc.GetTags();
            var moviePlot = movieDoc.GetPlot();
            var movieSet = movieDoc.GetSet();
            var movieDirector = movieDoc.GetDirector();
            var movieStudio = movieDoc.GetStudio();
            var movieReleaseDate = movieDoc.GetReleaseDate();
            var movieYear = !string.IsNullOrEmpty(movieReleaseDate) && DateTime.TryParse(movieReleaseDate, out var rd) ? rd.Year : (int?)null;

            var targetTitle = targetDoc.GetTitle();
            var targetOriginalTitle = targetDoc.GetOriginalTitle();
            var targetSortTitle = targetDoc.GetSortTitle();
            var targetActors = targetDoc.GetActors();
            var targetGenres = targetDoc.GetGenres();
            var targetTags = targetDoc.GetTags();

            // --- title ---
            string newTitle = targetTitle;
            if (DirNameFormatRegex.IsMatch(targetTitle))
                newTitle = movieTitle;

            // --- originaltitle ---
            string newOriginalTitle = targetOriginalTitle;
            if (DirNameFormatRegex.IsMatch(targetOriginalTitle))
                newOriginalTitle = movieOriginalTitle;

            // --- sorttitle ---
            string newSortTitle = targetSortTitle;
            if (SortTitleFormatRegex.IsMatch(targetSortTitle))
                newSortTitle = movieSortTitle;

            string newSet = movieSet;
            //if (!string.IsNullOrEmpty(movieSet))
            //{ }

            string newDirector = movieDirector;
            string newStudio = movieStudio;
            string newReleaseDate = movieReleaseDate;

            // --- actors ---
            List<(string Name, string Type)> newActors = targetActors;
            if (movieActors != null && movieActors.Count == 1)
            {
                var actorName = movieActors[0].Name;
                if (IsJapanese(actorName))
                {
                    // 读取爷爷级目录名（当前 nfo 所在目录的父目录的父目录）
                    var dirInfo = new DirectoryInfo(Path.GetDirectoryName(targetNfoPath));
                    var grandParentDir = dirInfo.Parent;
                    if (grandParentDir != null && IsChinese(grandParentDir.Name))
                    {
                        newActors = new List<(string Name, string Type)>
                        {
                            (grandParentDir.Name, movieActors[0].Type ?? "Actor")
                        };
                    }
                    else
                    {
                        // 回退：调用 ProcessNfoFile 爬取结果（用爬取结果的 actor）
                        var scraped = await GetScrapedActorsAsync(targetNfoPath);
                        if (scraped != null && scraped.Count > 0)
                            newActors = scraped.Select(a => (a, "Actor")).ToList();
                        else
                            newActors = movieActors;
                    }
                }
                else
                {
            ...
            else if (movieActors != null && movieActors.Count > 0)
            {
                newActors = movieActors;
            }

            // --- genre、tag ---
            var newGenres = movieGenres;
            var newTags = movieTags;

            bool needScrapedGenres = movieGenres == null || movieGenres.Count < 3;
            bool needScrapedTags = movieTags == null || movieTags.Count < 3;

            // genre 或 tag 不足 3 个时统一调用一次爬取
            if (needScrapedGenres || needScrapedTags)
            {
                var scrapedData = await GetScrapedGenresTagsAsync(targetNfoPath);
                if (needScrapedGenres && scrapedData.Genres != null && scrapedData.Genres.Count > 0)
                    newGenres = scrapedData.Genres;
                if (needScrapedTags && scrapedData.Tags != null && scrapedData.Tags.Count > 0)
                    newTags = scrapedData.Tags;
            }
            ...
            var javId = newSortTitle.Replace("-UC", "").Replace("-C", "").Replace("-U", "");
            // 对 genres 和 tags 进行术语替换和移除处理
            newGenres = NormalizeTagList(newGenres ?? targetGenres, [.. newActors.Select(c => c.Name)]);
            newTags = NormalizeTagList(newTags ?? targetTags, [.. newActors.Select(c => c.Name)]);

            // --- sorttitle 后缀处理 ---
            // 根据 title 中的前缀（中字/无码/中字无码）为 sorttitle 追加对应后缀
            // 检测依据：targetTitle（即文件名）中的方括号前缀部分
            var titleForPrefix = targetTitle ?? string.Empty;
            if (titleForPrefix.Contains("中字无码") || titleForPrefix.Contains("無碼中字"))
            {
                if (!newSortTitle.EndsWith("-UC", StringComparison.OrdinalIgnoreCase))
                    newSortTitle = newSortTitle + "-UC";
            }
            else if (titleForPrefix.Contains("中字"))
            {
                if (!newSortTitle.EndsWith("-C", StringComparison.OrdinalIgnoreCase))
                    newSortTitle = newSortTitle + "-C";
            }
            else if (titleForPrefix.Contains("无码") || titleForPrefix.Contains("無碼"))
            {
                if (!newSortTitle.EndsWith("-U", StringComparison.OrdinalIgnoreCase))
                    newSortTitle = newSortTitle + "-U";
            }

            // 如果 plot 为空则使用 movie.nfo 的 plot
            var newPlot = targetDoc.GetPlot();
            if (string.IsNullOrEmpty(newPlot))
            {
                newPlot = movieDoc.GetPlot();
                if (!string.IsNullOrEmpty(newPlot))
                    newPlot = moviePlot;
            }

            // 如果依旧为空，根据 studio 调用对应刺削器获取 plot
            if (string.IsNullOrEmpty(newPlot))
            {
                try
                {
                    Entities.JavVideo scrapedForPlot = null;
                    var studio = newStudio ?? string.Empty;
                    // 先检查本地缓存
                    var cachedVideo = await TryGetCachedJavVideoAsync(javId);
                    if (cachedVideo != null)
                    {
                        Console.WriteLine($"[缓存命中] 从本地缓存读取番号 {javId} 的影片信息。");
                        scrapedForPlot = cachedVideo;
                    }
                    else
                    {
                        if (studio.Contains("HEYZO", StringComparison.OrdinalIgnoreCase) ||
                        studio.Contains("Heyzo", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetHeyzoMetadata(javId);
                        }
                        else if (studio.Contains("カリビアンコムプレミアム", StringComparison.OrdinalIgnoreCase) ||
                                 studio.Contains("Caribbeancom PR", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetCaribbeanPRMetadata(javId);
                        }
                        else if (studio.Contains("一本道", StringComparison.OrdinalIgnoreCase) ||
                                 studio.Contains("Caribbeancom PR", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).Get1PondoMetadata(javId);
                        }
                        else if (studio.Contains("カリビアンコム", StringComparison.OrdinalIgnoreCase) ||
                                 studio.Contains("Caribbean", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetCaribbeanMetadata(javId);
                        }
                        else if (studio.Contains("キャットウォーク", StringComparison.OrdinalIgnoreCase) ||
                                studio.Contains("スーパーモデルメディア", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetAVEMetadata(javId);

                        }
                        else if (studio.Contains("キャットウォーク", StringComparison.OrdinalIgnoreCase) ||
                                studio.Contains("pacopacomama", StringComparison.OrdinalIgnoreCase))
                        {
                            scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetPacopacomamaMetadata(javId);

                        }
                        else
                        {
                            // 通用：先尝试 DMM，再尝试 JavDB
                            scrapedForPlot = await new DMM(_loggerFactory).SearchAndParseJavVideo(javId);
                            if (scrapedForPlot == null || string.IsNullOrEmpty(scrapedForPlot.Plot))
                                scrapedForPlot = await new JavDB(_loggerFactory).SearchAndParseJavVideo(javId);
                        }

                        if (scrapedForPlot != null && !string.IsNullOrEmpty(scrapedForPlot.Title) && !string.IsNullOrEmpty(scrapedForPlot.Plot))
                        {
                            if (string.IsNullOrEmpty(scrapedForPlot.Date))
                            {
                                var idStr = (string)javId;
                                if (!string.IsNullOrEmpty(idStr))
                                {
                                    var digitsMatch = System.Text.RegularExpressions.Regex.Match(idStr, @"\d{6}");
                                    if (digitsMatch.Success)
                                    {
                                        var d = digitsMatch.Value;
                                        if (DateTime.TryParseExact(d, "MMddyy",
                                            System.Globalization.CultureInfo.InvariantCulture,
                                            System.Globalization.DateTimeStyles.None,
                                            out var parsedDate)
                                            && parsedDate.Year >= 2005 && parsedDate <= DateTime.Today)
                                        {
                                            scrapedForPlot.Date = parsedDate.ToString("yyyy-MM-dd");
                                            //scrapedForPlot.Year = parsedDate.Year.ToString();
                                            Console.WriteLine($"  -> 从番号 {idStr} 提取发行日期: {scrapedForPlot.Date}");
                                        }
                                    }
                                }
                            }
                            await SaveJavVideoCacheAsync(javId, scrapedForPlot);
                            newPlot = scrapedForPlot.Plot;
                            Console.WriteLine($"  -> 从刺削器获取到 plot。");
                        }
                    }
                }
                catch (Exception ex)
                {
                    LogError($"获取 plot 失败", ex);
                }
            }

            // 保存到目标 nfo
            targetDoc.SaveMetadata(
                newTitle,
                newOriginalTitle,
                newSortTitle,
                newPlot,
                javId,
                [.. newActors.Select(a => a.Name)],
                newGenres,
                newTags,
                newSet,
                newDirector,
            ...
            Console.WriteLine($"  -> 已从 movie.nfo 复制字段到: {Path.GetFileName(targetNfoPath)}");
        }

        /// <summary>
        /// 调用爬虫获取指定 nfo 文件对应的演员列表（用于 actor 回退）。
        /// </summary>
        private async Task<List<string>> GetScrapedActorsAsync(string nfoFilePath)
        {
            try
            {
                var nfoDoc = new NfoDocument(nfoFilePath);
                var metadata = await GetMetadataFromNfo(nfoDoc);
                return metadata?.JavVideo?.Actors;
            }
            catch
            {
                return null;
        ...
        /// <summary>
        /// 调用爬虫获取指定 nfo 文件对应的 genre/tag（用于 genre、tag 不足时的回退）。
        /// </summary>
        private async Task<(List<string> Genres, List<string> Tags)> GetScrapedGenresTagsAsync(string nfoFilePath)
        {
            try
            {
                var nfoDoc = new NfoDocument(nfoFilePath);
                var metadata = await GetMetadataFromNfo(nfoDoc);
                return (metadata?.JavVideo?.Genres, metadata?.JavVideo?.Tags);
            }
            catch
            {
                return (null, null);
        ...
        private static bool IsJapanese(string text)
        {
            ...
            return text.Any(c =>
                (c >= '\u3040' && c <= '\u309F') || // 平假名
                (c >= '\u30A0' && c <= '\u30FF') || // 片假名
                (c >= '\u4E00' && c <= '\u9FFF')    // CJK 统一汉字（包含部分日文汉字）
            );
        }

        /// <summary>
        /// 判断字符串是否主要由中文字符构成（CJK 汉字占比超过 50%）。
        /// </summary>
        private static bool IsChinese(string text)
        {
            if (string.IsNullOrEmpty(text)) return false;
            int chineseCount = text.Count(c => c >= '\u4E00' && c <= '\u9FFF');
            return chineseCount > 0 && (double)chineseCount / text.Length > 0.5;
        }


        /// <summary>
        /// 对标签/类型列表进行规范化处理：使用 <see cref="TagReplaceConfig"/> 中的配置（替换 + 移除）。
        /// </summary>
        /// <param name="tags">原始标签列表。</param>
        /// <param name="actors">演员列表（预留参数，供后续扩展）。</param>
        /// <returns>处理后去重的标签列表。</returns>
        private List<string> NormalizeTagList(List<string> tags, List<string> actors)
        {
            if (tags == null || tags.Count == 0)
                return tags ?? new List<string>();

            return TagNormalizer.Normalize(tags, _tagReplaceConfig);
        }
        ...
        private async Task<MetadataInfo> GetMetadataFromNfo(NfoDocument nfoDocument)
        {
            ...
            var tags = nfoDocument.GetTags().Count < 2 ? nfoDocument.GetGenres() : nfoDocument.GetTags();
            var newStudio = nfoDocument.GetStudio();
            ...
            // 先检查本地缓存
            var cachedVideo = await TryGetCachedJavVideoAsync(javId.Id);
            if (cachedVideo != null)
            {
                Console.WriteLine($"[缓存命中] 从本地缓存读取番号 {javId.Id} 的影片信息。");
                javVideo = cachedVideo;
            }
            else
            {
                // 根据配置优先级尝试不同的刮削器
                foreach (var scraperName in _scraperConfig.PreferredScrapers ?? [])
                {
                    try
                    {
                        ...
                        if (string.Equals(scraperName, "DMM", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (javVideo != null && javVideo.Samples.Any())
                            {
                            ...
                            Console.WriteLine($"使用 DMM 刮片器刮削 {javId} 影片信息。");
                        }
                        else if (string.Equals(scraperName, "Jav123", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            Console.WriteLine($"使用 Jav123Scraper 刮片器刮削 {javId} 影片信息。");
                        }
                        else if (string.Equals(scraperName, "JavDB", StringComparison.OrdinalIgnoreCase))
                        {
                            javVideo = await new JavDB(_loggerFactory).SearchAndParseJavVideo($"{javId}");
                            Console.WriteLine($"使用 JavDB 刮片器刮削 {javId} 影片信息。");
                        }
                        else if (string.Equals(scraperName, "JavBus", StringComparison.OrdinalIgnoreCase))
                        {
                            javVideo = await new JavBus(_loggerFactory).ParsePage($"https://www.javbus.com//{javId}");
                            Console.WriteLine($"使用 JavBus 刮片器刮削 {javId} 影片信息。");
                        }
                        else if (string.Equals(scraperName, "JavCaptain", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            Console.WriteLine($"使用 JavCaptain 刮片器刮削 {javId} 影片信息。");
                        }
                ...
                // 刮削成功则保存到本地缓存
                if (javVideo != null && !string.IsNullOrEmpty(javVideo.Title))
                {
                    await SaveJavVideoCacheAsync(javId.Id, javVideo);
                }
                else
                {
                    try
                    {
                        ...
                        {
                            if (studio.Contains("HEYZO", StringComparison.OrdinalIgnoreCase) ||
                            studio.Contains("Heyzo", StringComparison.OrdinalIgnoreCase) || javId.ToString().ToLower().Contains("heyzo"))
                            {
                                scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetHeyzoMetadata(javId);
                                Console.WriteLine($"使用 Heyzo 刮片器刮削 {javId} 影片信息。");
                            }
                            else if (studio.Contains("カリビアンコムプレミアム", StringComparison.OrdinalIgnoreCase) ||
                                     studio.Contains("Caribbeancom PR", StringComparison.OrdinalIgnoreCase))
                            {
                                scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetCaribbeanPRMetadata(javId);
                                if (scrapedForPlot != null)
                                {
                                    Console.WriteLine($"使用 Caribbean PR 刮片器刮削 {javId} 影片信息，刮削成功。");
                                }
                                else
                                {
                                    Console.WriteLine($"使用 Caribbean PR 刮片器刮削 {javId} 影片信息，刮削失败。");
                                }
                            }
                            else if (studio.Contains("一本道", StringComparison.OrdinalIgnoreCase) ||
                                     studio.Contains("1Pondo", StringComparison.OrdinalIgnoreCase))
                            {
                                scrapedForPlot = await new UncensoredScraper(_loggerFactory).Get1PondoMetadata(javId);
                                Console.WriteLine($"使用 1Pondo 刮片器刮削 {javId} 影片信息。");
                            }
                            else if (studio.Contains("カリビアンコム", StringComparison.OrdinalIgnoreCase) ||
                                     studio.Contains("Caribbean", StringComparison.OrdinalIgnoreCase))
                            {
                                scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetCaribbeanMetadata(javId);
                                Console.WriteLine($"使用 Caribbean 刮片器刮削 {javId} 影片信息。");
                            }
                            else if (studio.Contains("キャットウォーク", StringComparison.OrdinalIgnoreCase) ||
                                    studio.Contains("スーパーモデルメディア", StringComparison.OrdinalIgnoreCase)||
                                    studio.Contains("スカイハイエンターテインメント", StringComparison.OrdinalIgnoreCase))
                            {
                                scrapedForPlot = await new UncensoredScraper(_loggerFactory).GetAVEMetadata(javId);
                                Console.WriteLine($"使用 AVE 刮片器刮削 {javId} 影片信息。");
                            ...
                            if (scrapedForPlot != null && !string.IsNullOrEmpty(scrapedForPlot.Title) && !string.IsNullOrEmpty(scrapedForPlot.Plot))
                            {
                                // 处理 Caribbean、pacopacomama、Caribbeancom PR 没有刮削到发行日期的问题
                                // 从"082815_143"或"072616-216"格式番号中提取发行日期：前6位 MMDDYY
                                if (string.IsNullOrEmpty(scrapedForPlot.Date))
                                {
                                    var idStr = (string)javId;
                                    if (!string.IsNullOrEmpty(idStr))
            ...
            // 如果介绍为空，则再刮削一次
            if (String.IsNullOrEmpty(javVideo.Plot))
            {
                var newJavVideo = await new DMM(_loggerFactory).SearchAndParseJavVideo(javId.Id);
                Console.WriteLine($"本地缓存中缺少影片介绍，需要重新从 DMM 刮削。");
                if (newJavVideo != null)
                {
                    javVideo.Plot = newJavVideo.Plot;
                    Console.WriteLine($"从 DMM 刮削到影片介绍，重新缓存到本地。");
                    await SaveJavVideoCacheAsync(javId.Id, javVideo);
                }
            }
        ...
        private async Task ProcessMetadata(MetadataInfo metadata, NfoDocument nfoDocument, FileInfo fileInfo)
        {
            ...
            // --- title ---
            string newTitle = nfoDocument.GetTitle();
            if (DirNameFormatRegex.IsMatch(nfoDocument.GetTitle()))
            {
                newTitle = videoInfo.VideoTitle;
            }

            //// --- originaltitle ---
            string newOriginalTitle = nfoDocument.GetOriginalTitle();
            if (DirNameFormatRegex.IsMatch(nfoDocument.GetOriginalTitle()))
            {
                newOriginalTitle = videoInfo.VideoOriginalTitle;
            }

            //// --- sorttitle ---
            string newSortTitle = nfoDocument.GetSortTitle();
            if (SortTitleFormatRegex.IsMatch(nfoDocument.GetSortTitle()))
            {
                newSortTitle = videoInfo.VideoSortTitle;
            }

            string newPlot = nfoDocument.GetPlot();
            if (String.IsNullOrEmpty(nfoDocument.GetPlot()) || nfoDocument.GetPlot().Length < 100)
            {
                newPlot = videoInfo.VideoPlot;
            }

            string newSet = nfoDocument.GetSet();
            if (String.IsNullOrEmpty(newSet))
            {
                newSet = metadata.JavVideo.Set;
            }

            string newDirector = nfoDocument.GetDirector();
            if (String.IsNullOrEmpty(newDirector))
            {
                newDirector = metadata.JavVideo.Director;
            }


            string newStudio = nfoDocument.GetStudio();
            if (String.IsNullOrEmpty(newStudio))
            {
                newStudio = metadata.JavVideo.Studio;
            }

            newTitle = newTitle.Replace("[中字无码]", "").Replace("[无码]", "").Replace("[中字]", "").Trim();

            if (fileInfo.Name.Contains("中字无码") && !newTitle.Contains("中字无码"))
            {
                newTitle = $"[中字无码] {newTitle}";
            }

            if (fileInfo.Name.Contains("无码") && !newTitle.Contains("无码"))
            {
                newTitle = $"[无码] {newTitle}";
            }

            if (fileInfo.Name.Contains("中字") && !newTitle.Contains("中字"))
            {
                newTitle = $"[中字] {newTitle}";
            }

            newTitle = newTitle.Replace("【プレコレ】", "").Replace("【アウトレット】", "").Replace("【ベストヒッツ】", "");
            newOriginalTitle = newOriginalTitle.Replace("【プレコレ】", "").Replace("【アウトレット】", "").Replace("【ベストヒッツ】", "");

            // --- sorttitle 后缀处理 ---
            // 根据 title 中的前缀（中字/无码/中字无码）为 sorttitle 追加对应后缀
            // 检测依据：targetTitle（即文件名）中的方括号前缀部分
            var titleForPrefix = newTitle ?? string.Empty;
            if (titleForPrefix.Contains("中字无码") || titleForPrefix.Contains("無碼中字"))
            {
                //if (!newSortTitle.EndsWith("-UC", StringComparison.OrdinalIgnoreCase))
                newSortTitle = metadata.JavId + "-UC";
            }
            else if (titleForPrefix.Contains("中字"))
            {
                //if (!newSortTitle.EndsWith("-C", StringComparison.OrdinalIgnoreCase))
                newSortTitle = metadata.JavId + "-C";
            }
            else if (titleForPrefix.Contains("无码") || titleForPrefix.Contains("無碼"))
            {
                //if (!newSortTitle.EndsWith("-U", StringComparison.OrdinalIgnoreCase))
                newSortTitle = metadata.JavId + "-U";
            }

            // 处理封面图片
            await ProcessCoverImage(metadata, videoInfo);
            metadata.Tags.AddRange(metadata.Genres);
            metadata.Tags.AddRange(metadata.JavVideo.Tags);

            var newTags = metadata.Tags ?? [];


            newTags = ApplyActorReplacement(newTags);

            metadata.Genres = NormalizeTagList(newTags, videoInfo.VideoActors);
            metadata.Tags = NormalizeTagList(newTags, videoInfo.VideoActors);
            ...
            // 保存前 对演员名字进行处理
            //videoInfo.VideoActors = ApplyActorReplacement(videoInfo.VideoActors);

            // 保存更新后的 nfo 文件
            // 注意：此处只进行基本的元数据保存，后续 FixNfoTagsAsync 会进一步处理标签和文件名
            nfoDocument.SaveMetadata(newTitle, newOriginalTitle, newSortTitle, newPlot, videoInfo.VideoId,
                videoInfo.VideoActors, metadata.Genres, metadata.Tags, newSet, newDirector, newStudio, metadata.JavVideo.GetYear(), metadata.JavVideo.Date);

            // 更新 metadata 对象用于打印
            metadata.Title = newTitle;
            metadata.OriginalTitle = newOriginalTitle;
        ...
        private VideoInfo CreateVideoInfo(MetadataInfo metadata, NfoDocument nfoDocument, FileInfo fileInfo)
        {
            ...
            return new VideoInfo
            {
                ...
                VideoDirector = metadata.JavVideo.Director,
        ...
        private async Task ProcessCoverImage(MetadataInfo metadata, VideoInfo videoInfo)
        {
            ...
            {
                System.IO.File.Copy(coverDict["fanart"], saveName, true);
            }
            else
            {
        ...
        private class VideoInfo
        {
            ...
            public string VideoStudio { get; set; }
