                ...
                using (var ms = new MemoryStream(bytes))
                {
                    ...
                    using (var inputStream = new SKManagedStream(ms))
                    {
                        using (var bitmap = SKBitmap.Decode(inputStream))
                        {
                            ...
                            var posterWidth = height * 2 / 3; //封面宽度

                            if (posterWidth < width) //需要剪裁
                            {
                                var x = await GetBaiduBodyAnalysisResult(bytes, url);
                                var start_w = width - posterWidth; //默认右边

                                if (x > 0) // 百度人体识别，中心点位置
                                {
                                    if (x + posterWidth / 2 > width) // 右边
                                        start_w = width - posterWidth;
                                    else if (x - posterWidth / 2 < 0)// 左边
                                        start_w = 0;
                                    else // 居中
                                        start_w = (int)x - posterWidth / 2;
                                }

                                var image = SKImage.FromBitmap(bitmap);

                                var subset = image.Subset(SKRectI.Create(start_w, 0, posterWidth, height));
                                var encodedData = subset.Encode(SKEncodedImageFormat.Jpeg, 90);
                                logger?.Info($"{nameof(CutImage)}: Already cut {width} x {height} --> start_w: {start_w}");
                                return new HttpResponseInfo()
                                {
                            ...
                            logger?.Info($"{nameof(CutImage)}: not need to cut. {width}x{height}");
                            return null;
                        }
                    }
                }
