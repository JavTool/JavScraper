...
namespace JavScraper.Baidu
{
    ...
    public static class BaiduFanyiService
    {
        ...
        private static JavConfig _javConfig;

        static BaiduFanyiService()
        {
        ...
        {
            if (_javConfig.EnableBaiduFanyi == false)
                return null;

            // 源语言
            string from = "auto";
            // 目标语言
            string to = _javConfig.BaiduFanyiLanguage?.Trim();
            if (string.IsNullOrWhiteSpace(to))
                to = "zh";

            string appId = _javConfig.BaiduFanyiApiKey;
            string secretKey = _javConfig.BaiduFanyiSecretKey;
