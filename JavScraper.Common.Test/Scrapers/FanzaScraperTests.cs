...
namespace JavScraper.Common.Test.Scrapers
{
    ...
    /// FanzaScraper 单元测试类
    /// </summary>
    public class FanzaScraperTests : TestBase
    {
        private readonly ITestOutputHelper _output;
        private readonly FanzaScraper _scraper;

        public FanzaScraperTests(ITestOutputHelper output)
        {
            _output = output;
            _scraper = (FanzaScraper)CreateScraper("fanza");
        }

        [Fact]
        public void Name_Should_BeFanza()
        {
            Assert.Equal("fanza", _scraper.Name, ignoreCase: true);
        }
        ...
        public void DefaultBaseUrl_Should_NotBeEmpty()
        {
            ...
            // Fanza 使用 dmm.co.jp 域名
            Assert.Contains("dmm", _scraper.DefaultBaseUrl, StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            var code = "abp00782"; // Fanza 使用小写无连字符格式
        ...
        {
            var video = await _scraper.ScrapeAsync("invalid999");
